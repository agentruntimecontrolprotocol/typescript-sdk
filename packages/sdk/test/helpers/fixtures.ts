...
export function makeBearerVerifier(
  extra: ReadonlyMap<string, { principal: string }> = new Map(),
) {
...
export function makePairedHarness(
  ...
  const client = new ARCPClient({
    client: TEST_CLIENT,
    capabilities: { encodings: ["json"], ...clientOpts.capabilities },
    authScheme: "bearer",
    token: TEST_TOKEN,
    logger: silentLogger,
...
/**
 * Wait until `getter()` returns a non-null value and return it.
 */
