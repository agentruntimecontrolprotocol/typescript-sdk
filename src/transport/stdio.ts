...
export class StdioTransport implements Transport {
  ...
  public static fromChild(child: {
    ...
    if (child.stdin === null || child.stdout === null) {
      throw new InvalidArgumentError("Child process must be spawned with stdin and stdout pipes");
    }
  ...
  public onFrame(handler: FrameHandler): void {
    if (this.handler !== null) {
      throw new InvalidArgumentError("StdioTransport already has a frame handler");
    }
    this.handler = handler;
    this.rl = createInterface({ input: this.readable, crlfDelay: Number.POSITIVE_INFINITY });
