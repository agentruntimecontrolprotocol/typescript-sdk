...
import { validateAgainstSchema } from "../../src/util/json-schema.js";
...
describe("validateAgainstSchema (JSON-Schema subset)", () => {
  ...
  it("validates type=number/integer", () => {
    ...
    expect(validateAgainstSchema(Number.NaN, { type: "number" })).toHaveLength(1);
  });
  ...
  it("validates string length", () => {
    expect(validateAgainstSchema("ab", { type: "string", minLength: 1, maxLength: 4 })).toEqual([]);
    expect(validateAgainstSchema("", { type: "string", minLength: 1 })).toHaveLength(1);
    expect(validateAgainstSchema("xxxxx", { type: "string", maxLength: 3 })).toHaveLength(1);
  });

  it("validates number bounds", () => {
    expect(validateAgainstSchema(5, { type: "number", minimum: 1, maximum: 10 })).toEqual([]);
    expect(validateAgainstSchema(-1, { type: "number", minimum: 0 })).toHaveLength(1);
    expect(validateAgainstSchema(99, { type: "number", maximum: 10 })).toHaveLength(1);
  });
  ...
  it("validates nested properties", () => {
    ...
    expect(validateAgainstSchema({ outer: { inner: "x" } }, schema)).toHaveLength(1);
  ...
  it("compound: object with properties + arrays + enums", () => {
    ...
    expect(validateAgainstSchema({ tags: ["a", "b"], count: 1 }, schema)).toEqual([]);
    expect(validateAgainstSchema({ tags: ["c"], count: 1 }, schema).length).toBeGreaterThan(0);
    expect(validateAgainstSchema({ count: 5 }, schema).length).toBeGreaterThan(0);
  });
});
