...
| `history: true` replays buffered events; each replayed event uses the *subscriber's* `event_seq` | Implemented | `packages/runtime/src/server.ts:handleJobSubscribe` + `forwardEventToSubscriber` (uses `sub.nextEventSeq()`) |
