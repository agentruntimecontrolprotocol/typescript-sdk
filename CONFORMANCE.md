...
This SDK targets [ARCP v1.1](../spec/docs/draft-arcp-1.1.md), the
...
| §5.1 Unknown top-level fields MUST be ignored                                                 | Implemented | `RoundTripEnvelopeSchema` decodes through Effect Schema with unknown keys preserved via `Schema.Record`     |
...
| §6.1 Reject missing/invalid token with `session.error` + close                                                                 | Implemented | `packages/runtime/src/server.ts:authenticateHello` → `SessionContext.emitSessionError`/`terminate`               |
...
| §6.2 Runtime MUST issue `resume_token` (≥128 bits entropy); client MUST treat as credential                                    | Implemented | `packages/runtime/src/stores.ts:newResumeToken` uses 32 random bytes (256 bits)                                  |
| §6.2 `resume_token` rotated on every welcome                                                                                   | Implemented | `packages/runtime/src/server-resume.ts:handleResume` writes a fresh token to `resumeStore` and emits in the new welcome |
...
| §6.3 On resume, replay `event_seq > last_event_seq`, issue new welcome with same `session_id`, resume streaming                | Implemented | `packages/runtime/src/server-resume.ts:handleResume` reads via `EventLog.readSinceSeq` and re-sends frames       |
| §6.3 `RESUME_WINDOW_EXPIRED` on stale resume                                                                                   | Implemented | `packages/core/src/errors.ts:ResumeWindowExpiredError`; emitted in `server-resume.ts:handleResume`               |
| §6.7 Clean close via `session.bye { reason }`                                                                                  | Implemented | `packages/core/src/messages/session.ts:SessionByePayloadSchema`                                                  |
| §6.7 Either side MAY initiate; no further messages post-bye                                                                    | Implemented | `packages/runtime/src/session-context.ts:SessionContext.terminate` is idempotent                                 |
...
| §8.3 Sequence numbers SESSION-scoped (single space across concurrent jobs)                                                   | Implemented | `packages/runtime/src/session-context.ts:SessionContext.nextEventSeq`                                       |
...
| Requirement                                                                                               | Status      | Location                                                                                                  |
| --------------------------------------------------------------------------------------------------------- | ----------- | --------------------------------------------------------------------------------------------------------- |
| §9.1 Lease is IMMUTABLE, granted at submit; capability → glob pattern[]                                   | Implemented | `packages/core/src/messages/lease-schema.ts:LeaseSchema`; `packages/runtime/src/lease.ts:validateLeaseShape` |
...
| §10.1 Runtime responds with `job.accepted { job_id, parent_job_id, delegate_id, lease }`                                          | Implemented | `packages/runtime/src/job-runner.ts:createDelegateJob`                                           |
| §10.2 Subset validation surfaces failure as `tool_result` event on PARENT with `LEASE_SUBSET_VIOLATION`                           | Implemented | `packages/runtime/src/job-runner.ts:makeDelegateInterceptor`                                     |
| §10.3 Delegated jobs inherit parent `trace_id`; new span MAY be created                                                           | Implemented | `packages/runtime/src/job-runner.ts:createDelegateJob`                                           |

## §11. Trace Propagation

| Requirement                                                                                                                         | Status      | Location                                              |
| ----------------------------------------------------------------------------------------------------------------------------------- | ----------- | ----------------------------------------------------- |
| §11 `trace_id` is W3C 32-hex; client SHOULD include on submit; runtime MUST mint if absent, echo on `job.accepted.payload.trace_id` | Implemented | `packages/runtime/src/job-runner.ts:handleJobSubmit`  |
...
The v1.0 set is the twelve codes below. v1.1 adds three more
(`AGENT_VERSION_NOT_AVAILABLE`, `LEASE_EXPIRED`, `BUDGET_EXHAUSTED`) — see
the v1.1 additions section near the end of this document. The canonical
list of all 15 lives at `packages/core/src/errors.ts:ERROR_CODES`.
...
ARCP v1.1 is additive over v1.0. v1.0 wire frames carry `arcp: "1"`;
v1.1 frames carry `arcp: "1.1"` (`PROTOCOL_VERSION` in
`packages/core/src/version.ts`). New messages, fields, event kinds,
lease constraints, and error codes are negotiated via the v1.1
`features` capability list in `session.hello`/`session.welcome`.

## Feature negotiation matrix

| Feature flag       | Spec ref | Status      | Where the flag is consumed                                                                                                                               |
| ------------------ | -------- | ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `heartbeat`        | §6.4     | Implemented | `packages/runtime/src/session-context.ts:SessionContext.startHeartbeat`; client responds to ping in `packages/client/src/client-dispatch.ts`             |
| `ack`              | §6.5     | Implemented | `packages/runtime/src/server.ts:registerSessionControlHandlers` (`session.ack` handler) + `SessionContext.recordAck`; client `ARCPClient.ack` / `autoAck` |
| `list_jobs`        | §6.6     | Implemented | `packages/runtime/src/server-subscribe.ts:handleListJobs`; `ARCPClient.listJobs`                                                                         |
| `subscribe`        | §7.6     | Implemented | `packages/runtime/src/server-subscribe.ts:handleJobSubscribe`; `ARCPClient.subscribe`                                                                    |
| `lease_expires_at` | §9.5     | Implemented | `packages/runtime/src/lease.ts:validateLeaseConstraints`; expiry watchdog in `job-runner.ts:runHandler`                                                  |
...
| §6.2 Effective feature set is the intersection of both lists                                                           | Implemented | `packages/core/src/version.ts:intersectFeatures`; `packages/runtime/src/server.ts:makeNegotiatedCapabilities`; `packages/client/src/client.ts:awaitHandshake`  |
...
| `session.welcome.payload.heartbeat_interval_sec` advertised                                                                     | Implemented | `packages/core/src/messages/session.ts:SessionWelcomePayloadSchema`; `packages/runtime/src/server.ts:buildWelcomePayload`          |
| Runtime periodically pings if outbound idle and replies to inbound pings; closes with `HEARTBEAT_LOST` after 2 silent intervals | Implemented | `packages/runtime/src/session-context.ts:SessionContext.startHeartbeat`/`heartbeatTick`                                            |
| Client periodically pings if idle and replies to runtime pings                                                                  | Implemented | `packages/client/src/client-dispatch.ts` (pong path); ping side TBD when long-lived idle is required                               |
| `session.ping`/`session.pong` NOT counted in `event_seq`                                                                        | Implemented | `packages/runtime/src/session-context.ts` (`INBOUND_DEDUPE_SKIP` set)                                                              |
...
| Runtime records the client's `last_processed_seq`                              | Implemented | `packages/runtime/src/session-context.ts:SessionContext.recordAck`                                                                                          |
| Runtime MAY emit a `back_pressure` `status` event when lag exceeds a threshold | Implemented | `packages/runtime/src/session-context.ts:SessionContext.maybeEmitBackPressure` (default threshold 1000, configurable via `ARCPServerOptions.backPressureThreshold`) |
...
| `session.ack` NOT counted in `event_seq`                                       | Implemented | `packages/runtime/src/session-context.ts` (`INBOUND_DEDUPE_SKIP` set)                                                                                       |
...
| Runtime echoes the request envelope's `id` as `request_id`                                                                                          | Implemented | `packages/runtime/src/server-subscribe.ts:handleListJobs`                                                         |
| Authorization defaults to same-principal-only; broader policy via `jobAuthorizationPolicy`                                                          | Implemented | `packages/runtime/src/server-subscribe.ts:defaultJobAuthorizationPolicy` + `ARCPServerOptions.jobAuthorizationPolicy` |
...
| Agent grammar `agent ::= name \| name "@" version` (names lowercase `[a-z0-9][a-z0-9._-]*`; versions `[a-zA-Z0-9.+_-]+`)   | Implemented | `packages/core/src/messages/execution.ts:parseAgentRef` + `formatAgentRef`                     |
| Bare name resolves to the default version (or unversioned handler)                                                         | Implemented | `packages/runtime/src/agent-registry.ts:AgentRegistry.resolve`                                 |
| `name@version` requires an exact match; otherwise `AGENT_VERSION_NOT_AVAILABLE` (`session.error`)                          | Implemented | `packages/runtime/src/job-runner.ts:handleJobSubmit` (calls `emitSessionError` per §13.7)      |
...
| Same-principal scope by default; deployment policy can broaden                                                                    | Implemented | `packages/runtime/src/server-subscribe.ts:defaultJobAuthorizationPolicy` + `jobAuthorizationPolicy` hook                          |
| `history: true` replays buffered events; each replayed event uses the _subscriber's_ `event_seq`                                  | Implemented | `packages/runtime/src/server-subscribe.ts:handleJobSubscribe` (uses `sub.nextEventSeq()`)                                         |
| Live events fan out to all subscribers in addition to the owning session                                                          | Implemented | `packages/runtime/src/session-context.ts:SessionContext.fanOutToSubscribers`                                                      |
| Subscribers MUST NOT have cancel authority — only the submitting session may cancel                                               | Implemented | `packages/runtime/src/server.ts:emitCancelTargetMissing` returns `PERMISSION_DENIED` when the session is a subscriber, not the submitter |
...
| Runtime MUST emit `job.error { final_status: "error", code: "LEASE_EXPIRED" }` when the lease elapses while running | Implemented | `packages/runtime/src/job-runner.ts:runHandler` (lease-expiry timer)                                                                             |
...
| Counters decrement on `metric` events whose `name` starts with `cost.` and whose `unit` matches a budgeted currency | Implemented | `packages/runtime/src/job.ts:Job.applyCostMetric` + `packages/runtime/src/job-runner.ts:metricInterceptor`             |
...
| Runtime MAY emit `cost.budget.remaining` metric events with debounce                                                | Implemented | `packages/runtime/src/job-runner.ts:metricInterceptor` + `Job.shouldEmitBudgetRemaining` (5 % threshold)               |
| `JobContext.budget` read-only snapshot of remaining counters                                                        | Implemented | `packages/runtime/src/job-context.ts:makeJobContext`                                                                   |
...
| `credentialProvisioner` requires `credentialStore`                              | Implemented | `packages/runtime/src/server.ts:ARCPServer` constructor (throws `TypeError` when only the provisioner is set)   |
...
| Sandboxed lease enforcement                           | The SDK ships `validateLeaseOp` and expects the agent (or a runtime-wrapping shim) to call it.                                                  | Agent authors call `validateLeaseOp({ lease: ctx.lease, capability, target, ctx: { constraints, budgetRemaining } })`.                |
